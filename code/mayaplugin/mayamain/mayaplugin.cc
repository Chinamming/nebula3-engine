...
MayaPlugin::Setup(MObject pluginObject)
{
    ...
    MFnPlugin fnPlugin(pluginObject, "Radon Labs GmbH", "3.0");
...
MayaPlugin::MELStartup()
{
    ...
    String toolkitPath = MayaDirectoryRegistry::Instance()->GetToolkitPath();

    // load the rlstartup.mel script
    String startupScript;
    startupScript.Format("%s/data/maya/scripts/rlnebula3.mel", toolkitPath.AsCharPtr());
    ...
    String cmdString;
    cmdString.Format("rlStartup3(\"%s\",\"%s\");", toolkitPath.AsCharPtr(), projPath.AsCharPtr());
