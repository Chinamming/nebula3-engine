...
CPaneDialog& CMainFrame::GetPanelDialog()
{
	return this->m_dlgPane;
}
...
void CMainFrame::OnFileOpen()
{
 	...
 	if( fileDlg.DoModal() == IDOK)
	{
		...
		if( filename.Find(strPath.AsCharPtr())!=-1)
		{
			...
			Resources::ResourceId resId(filename.GetBuffer());
			m_gameApp.LoadModel(resId);
			GraphicsInterface::Instance()->WaitForPendingResources();
			if( Models::ModelServer::Instance()->HasManagedModel(resId) )
			{
				const Ptr<ManagedModel>& managedModel = Models::ModelServer::Instance()->LoadManagedModel(resId);
				if( managedModel.isvalid() )
				{
					const Ptr<Model>& model = managedModel->GetModel();
					n_assert(model.isvalid());
					m_wndExplorerView.OnExpandModel(model);
					m_dlgPane.SetModel(model);
				}
			}
		}
		else
