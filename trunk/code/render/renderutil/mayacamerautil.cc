...
MayaCameraUtil::MayaCameraUtil() :
    ...
    orbiting(0.0f, 0.0f)
{
    this->Setup(point(0.0f, 0.0f, 0.0f), point(0.0f, 0.0f, 10.0f), vector(0.0f, 1.0f, 0.0f));
}
...
{
    const scalar defOrbitVelocity = 0.25f;
    const scalar defPanVelocity  = 0.08f;
    const scalar defZoomVelocity = 0.25f;
    const scalar minPanVelocity  = 0.08f;
    const scalar minZoomVelocity = 0.50f;

    // zooming and panning speed is greater when farther away
    scalar panVelocity = defPanVelocity * this->viewDistance;
    panVelocity = n_max(minPanVelocity, panVelocity);
    scalar zoomVelocity = defZoomVelocity * this->viewDistance;
    zoomVelocity = n_max(minZoomVelocity, zoomVelocity);
    ...
    if (this->zoomButton)
    {
        zoom += -this->mouseMovement.x() + this->mouseMovement.y();
    }
